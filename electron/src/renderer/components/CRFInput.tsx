...
export default function CRFInput({ value, onChange }: Props) {
  return (
    <div>
      <div style={{fontSize:11,color:'#8A96A8',marginBottom:8}}>18 = best / 28 = smallest</div>
      ...
      <div className="flex-r justify-between mar-t-4">
        <span style={{fontSize:11,color:'#8A96A8'}}>Best</span>
        <span style={{fontSize:13,fontWeight:600,color:'#4CC2F1'}}>CRF {value}</span>
        <span style={{fontSize:11,color:'#8A96A8'}}>Smaller</span>
      </div>
    </div>
  );
}
